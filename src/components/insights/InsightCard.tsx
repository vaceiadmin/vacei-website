...
const InsightCard = ({ article, index }: InsightCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col h-full bg-white rounded-2xl border border-gray-100 hover:border-primary-blue/30 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-primary-blue/5"
    >
      <Link href={`/insights/${article.slug}`} className="flex flex-col h-full p-6 md:p-8">
        ...
        <div className="flex items-center justify-between mb-4">
          <span
            className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium transition-colors duration-300 ${
              isHovered ? "bg-primary-blue text-white" : "bg-primary-blue/5 text-primary-blue"
            }`}
          >
            {article.category}
          </span>
        ...
        {/* Title */}
        <h3 className="text-xl font-bold text-text-heading mb-3 group-hover:text-primary-blue transition-colors duration-300 line-clamp-2">
          {article.title}
        </h3>
